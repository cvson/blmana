void t2kpre_quickcheck_nuabort_byrun_allesm_bymonitorx(Int_t run2plot, Int_t ithbunch){
...
//TH2F *hesmx = new TH2F("hesmx","",23,0,23,100,-10,10);
//TH2F *hesmx_all = new TH2F("hesmx_all","",46,0,23,100,-10,10);
//TH2F *haxis = new TH2F("hesmx_all0","",46,0,23,100,-10,10);
//haxis->GetYaxis()->SetRangeUser(-10,10);
//haxis->SetTitle();
...
pchain->Draw(Form("esmx_bunch[][%d]:Iteration$+1.0>>haxis(46,0,23,100,-10,10)",ithbunch),"ssemx[18]!=0","colz");
haxis->SetTitle();
haxis->GetXaxis()->SetTitle("ESM monitors");
haxis->GetYaxis()->SetTitle(Form("ESM X of bunch %d [mm]",ithbunch+1));
...
c1->cd();
anabeam->Draw(Form("esmx_bunch[][%d]:Iteration$+1>>hesmx",ithbunch),Form("spillnum==%d",spill.at(i)),"lsame");
hesmx->SetLineColor(gStyle->GetColorPalette(theColor));
leg0->AddEntry(hesmx,Form("Run %06d Spill%06d ",run.at(i),spill.at(i)),"l");
c1->cd();
c1->Modified();
}
//
...
c1->Modified();
leg0->Draw("same");
//c1->Print(Form("plots/r71_nuabort_run%07d_allesmx_bymonitor%02d.eps",run2plot,ithbunch));
c1->Print(Form("plots/r72_nuabort_run%07d_allesmx_bymonitor%02d.eps",run2plot,ithbunch));
pfile->Close();
}
