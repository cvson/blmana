void t2kpre_quickcheck_nuabort_byrun_esm_bybunchx(Int_t run2plot, Int_t ithbunch){
...
//TH2F *hesmx = new TH2F("hesmx","",23,0,23,100,-8,8);
//TH2F *hesmx_all = new TH2F("hesmx_all","",18,0,9,100,-8,8);
//TH2F *haxis = new TH2F("hesmx_all0","",18,0,9,100,-8,8);
//haxis->GetYaxis()->SetRangeUser(-8,8);
//haxis->SetTitle();
...
pchain->Draw(Form("esmx_bunch[%d]:Iteration$+1.0>>haxis(18,0,9,100,-8,8)",ithbunch),"ssemx[18]!=0","colz");
haxis->SetTitle();
haxis->GetXaxis()->SetTitle("By bunch");
haxis->GetYaxis()->SetTitle(Form("ESM %02d X [mm]",ithbunch+1));
...
c1->Modified();
leg0->Draw("same");
//c1->Print(Form("plots/r71_nuabort_run%07d_esm%02d_bybunchx.eps",run2plot,ithbunch));
c1->Print(Form("plots/r72_nuabort_run%07d_esm%02d_bybunchx.eps",run2plot,ithbunch));
pfile->Close();
}
