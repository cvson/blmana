...
for(int j=0; j<NTEST17324; j++){
    ...
    for(int j=0; j<NTEST17406; j++){
        gr_17406[j]= (TGraphErrors*)pfile17406->Get(Form("grtest_17406%d",j));
    }
    TGraphErrors *gr2fit_17406[NBLM];
    TH1F* hct_17406[NTEST17406];
    double ctmean_17406[NTEST17406];
    double ctrms_17406[NTEST17406];
    double adcmean_17406[NBLM][NTEST17406];
    double adcrms_17406[NBLM][NTEST17406];
    
    for(int j=0; j<NTEST17406; j++){
        
        hct_17406[j] = (TH1F*)pfile17406->Get(Form("hct17406_sam%d",j));
        ctmean_17406[j] = hct_17406[j]->GetMean()*1e-12;
        ctrms_17406[j] = hct_17406[j]->GetRMS()*1e-12;
        
        gr_17406[j]= (TGraphErrors*)pfile17406->Get(Form("grtest_17406%d",j));
        for (Int_t iblm=0; iblm<NBLM; ++iblm) {
            gr_17406[j]->GetPoint(iblm,blmindex[iblm],adcmean_17406[iblm][j]);
            adcrms_17406[iblm][j] = gr_17406[j]->GetErrorY(iblm);
        }
    }
...
gr2fit_17324[iblm]->Draw("PL same");
        ...
        //scan 20170406
        gr2fit_17406[iblm] = new TGraphErrors(NTEST17406,ctmean_17406,adcmean_17406[iblm],ctrms_17406,adcrms_17406[iblm]);
        gr2fit_17406[iblm]->SetMarkerStyle(8);
        ci = TColor::GetColor(colorRun[4]);
        gr2fit_17406[iblm]->SetMarkerColor(ci);
        gr2fit_17406[iblm]->SetLineColor(ci);
        gr2fit_17406[iblm]->Draw("PL same");
	...
	leg0->AddEntry(gr2fit_17324[iblm], "Beam scan 17.03.24", "pl");
     leg0->AddEntry(gr2fit_17406[iblm], "Beam scan 17.04.06", "p");
leg0->SetFillStyle(0);
	leg0->Draw();
...
gPad->Update();
 TPaveStats *st4 = (TPaveStats*)gr2fit_17324[51]->GetListOfFunctions()->FindObject("stats");
    ...
    gr2fit_17406[50]->SetMarkerColor(ci);
    gr2fit_17406[50]->SetLineColor(ci);
    gr2fit_17406[50]->Draw("PL same");
    
    
    
    ci = TColor::GetColor(colorRun[6]);
    gr2fit_17406[51]->SetMarkerColor(ci);
    gr2fit_17406[51]->SetLineColor(ci);
    gr2fit_17406[51]->Draw("PL same");
    ...
    leg0->AddEntry(gr2fit_17406[50], "Si 01 (~20cm), Apr.6", "p");
    leg0->AddEntry(gr2fit_17406[51], "Si 02 (~20cm), Apr.6", "p");
    leg0->SetFillStyle(0);
    leg0->Draw();
