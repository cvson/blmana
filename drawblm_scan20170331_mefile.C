...
for(int j=0; j<NTEST17324; j++){
    ...
    for(int j=0; j<NTEST17331; j++){
        gr_17331[j]= (TGraphErrors*)pfile17331->Get(Form("grtest_17331%d",j));
    }
    TGraphErrors *gr2fit_17331[NBLM];
    TH1F* hct_17331[NTEST17331];
    double ctmean_17331[NTEST17331];
    double ctrms_17331[NTEST17331];
    double adcmean_17331[NBLM][NTEST17331];
    double adcrms_17331[NBLM][NTEST17331];
    
    for(int j=0; j<NTEST17331; j++){
        
        hct_17331[j] = (TH1F*)pfile17331->Get(Form("hct17331_sam%d",j));
        ctmean_17331[j] = hct_17331[j]->GetMean()*1e-12;
        ctrms_17331[j] = hct_17331[j]->GetRMS()*1e-12;
        
        gr_17331[j]= (TGraphErrors*)pfile17331->Get(Form("grtest_17331%d",j));
        for (Int_t iblm=0; iblm<NBLM; ++iblm) {
            gr_17331[j]->GetPoint(iblm,blmindex[iblm],adcmean_17331[iblm][j]);
            adcrms_17331[iblm][j] = gr_17331[j]->GetErrorY(iblm);
        }
    }
    ...
    for(Int_t iblm=0; iblm<NBLM; ++iblm){
        ...
        if(iblm>49)gr2fit[iblm]->SetMaximum(150);//for Si detector
...
gr2fit_17324[iblm]->Draw("PL same");
        ...
        //scan 20170331
        gr2fit_17331[iblm] = new TGraphErrors(NTEST17331,ctmean_17331,adcmean_17331[iblm],ctrms_17331,adcrms_17331[iblm]);
        gr2fit_17331[iblm]->SetMarkerStyle(8);
        ci = TColor::GetColor(colorRun[4]);
        gr2fit_17331[iblm]->SetMarkerColor(ci);
        gr2fit_17331[iblm]->SetLineColor(ci);
        gr2fit_17331[iblm]->Draw("PL same");
	...
	leg0->AddEntry(gr2fit_17324[iblm], "Beam scan 17.03.24", "pl");
        leg0->AddEntry(gr2fit_17331[iblm], "Beam scan 17.03.31", "pl");
leg0->SetFillStyle(0);
	leg0->Draw();
    ...
    if(iblm>49)gr2fit_17315[50]->SetMaximum(150);//for Si detector
...
gPad->Update();
 TPaveStats *st4 = (TPaveStats*)gr2fit_17324[51]->GetListOfFunctions()->FindObject("stats");
    ...
    leg0->SetBorderSize(0);
    leg0->SetTextSize(0.045);
    leg0->AddEntry(gr2fit_17315[50], "Si 01 (20cm)", "pl");
    leg0->AddEntry(gr2fit_17315[51], "Si 02 (20cm)", "pl");
    leg0->AddEntry(gr2fit_17324[50], "Si 01 center", "pl");
    leg0->AddEntry(gr2fit_17324[51], "Si 02 center", "pl");
    leg0->SetFillStyle(0);
    leg0->Draw();
