void t2kpre_quickcheck_nuabort_byrun_esmx(Int_t run2plot){
...
//TH2F *hesmx = new TH2F("hesmx","",23,0,23,100,-5,5);
//TH2F *hesmx_all = new TH2F("hesmx_all","",46,0,23,100,-5,5);
//TH2F *haxis = new TH2F("hesmx_all0","",46,0,23,100,-5,5);
...
c1->cd();
pchain->Draw("esmx:Iteration$+1.0>>haxis(46,0,23,100,-5,5)","ssemx[18]!=0","colz");
haxis->SetTitle();
haxis->GetXaxis()->SetTitle("ESM monitors");
haxis->GetYaxis()->SetTitle("ESM X [mm]");
...
c1->cd();
anabeam->Draw("esmx:Iteration$+1>>hesmx",Form("spillnum==%d",spill.at(i)),"lsame");
...
c1->Modified();
leg0->Draw("same");
//c1->Print(Form("plots/r71_nuabort_run%07d_esmx.eps",run2plot));
c1->Print(Form("plots/r72_nuabort_run%07d_esmx.eps",run2plot));
pfile->Close();
}
