void drawblm_scan20170411_newcircuit_mefile(){
    ...
    TFile *pfile17411newci = new TFile("beamscan_20170411_newcircuit.root");
    new TCanvas;
...
for(int j=0; j<NTEST17324; j++){
    ...
    /***********************************/
    //scan 2017017411newci
    const Int_t NTEST17411newci = 1;
    TGraphErrors *gr_17411newci[NTEST17411newci];
    for(int j=0; j<NTEST17411newci; j++){
        gr_17411newci[j]= (TGraphErrors*)pfile17411newci->Get(Form("grtest_17411newci%d",j));
    }
    TGraphErrors *gr2fit_17411newci[NBLM];
    TH1F* hct_17411newci[NTEST17411newci];
    double ctmean_17411newci[NTEST17411newci];
    double ctrms_17411newci[NTEST17411newci];
    double adcmean_17411newci[NBLM][NTEST17411newci];
    double adcrms_17411newci[NBLM][NTEST17411newci];
    
    for(int j=0; j<NTEST17411newci; j++){
        
        hct_17411newci[j] = (TH1F*)pfile17411newci->Get(Form("hct17411newci_sam%d",j));
        ctmean_17411newci[j] = hct_17411newci[j]->GetMean()*1e-12;
        ctrms_17411newci[j] = hct_17411newci[j]->GetRMS()*1e-12;
        
        gr_17411newci[j]= (TGraphErrors*)pfile17411newci->Get(Form("grtest_17411newci%d",j));
        for (Int_t iblm=0; iblm<NBLM; ++iblm) {
            gr_17411newci[j]->GetPoint(iblm,blmindex[iblm],adcmean_17411newci[iblm][j]);
            adcrms_17411newci[iblm][j] = gr_17411newci[j]->GetErrorY(iblm);
        }
    }
...
gr2fit_17324[iblm]->Draw("PL same");
        ...
        //scan 2017017411newci
        gr2fit_17411newci[iblm] = new TGraphErrors(NTEST17411newci,ctmean_17411newci,adcmean_17411newci[iblm],ctrms_17411newci,adcrms_17411newci[iblm]);
        gr2fit_17411newci[iblm]->SetMarkerStyle(8);
        ci = TColor::GetColor(colorRun[4]);
        gr2fit_17411newci[iblm]->SetMarkerColor(ci);
        gr2fit_17411newci[iblm]->SetLineColor(ci);
        gr2fit_17411newci[iblm]->Draw("PL same");
    ...
    leg0->AddEntry(gr2fit_17411newci[iblm], "Si-01 new circuit 17.04.11 ", "p");
    leg0->SetFillStyle(0);
	leg0->Draw();
...
gPad->Update();
 TPaveStats *st4 = (TPaveStats*)gr2fit_17324[51]->GetListOfFunctions()->FindObject("stats");
    ...
    //new scan 2017017411newci
    ci = TColor::GetColor(colorRun[7]);
    gr2fit_17411newci[50]->SetMarkerColor(ci);
    gr2fit_17411newci[50]->SetLineColor(ci);
    gr2fit_17411newci[50]->Draw("PL same");
    
    ci = TColor::GetColor(colorRun[8]);
    gr2fit_17411newci[51]->SetMarkerColor(ci);
    gr2fit_17411newci[51]->SetLineColor(ci);
    gr2fit_17411newci[51]->Draw("PL same");
    ...
    leg0->AddEntry(gr2fit_17411newci[50], "Si 01 (~20cm), Apr.11, new circuit", "pl");
    leg0->AddEntry(gr2fit_17411newci[51], "Si 02 (~20cm), Apr.11, (Si01 new circuit)", "pl");
    leg0->SetFillStyle(0);
    leg0->Draw();
