void t2kpre_quickcheck_nuabort_byrun_angle(Int_t run2plot){
gROOT->ProcessLine(".x /home/cvson/rootlogon.C");
    gROOT->ProcessLine(".L /home/cvson/basicPlotUtil.C");
const Int_t NRGBs = 5;
    const Int_t NCont = 255;
double esm_2_1_pos = (8153.81 - 1848.82)/1000.;
...
c1->cd();
pchain->Draw(Form("(esmx_bunch[0]-esmx_bunch[1])/%f:Iteration$+1>>haxis(18,0,9,100,-1.,1.)",esm_2_1_pos),"ssemx[18]!=0","colz");
haxis->SetTitle();
haxis->GetXaxis()->SetTitle("By bunch");
haxis->GetYaxis()->SetTitle("Beam angle [mrad]");
...
c1->cd();
anabeam->Draw(Form("(esmx_bunch[0]-esmx_bunch[1])/%f:Iteration$+1>>hct",esm_2_1_pos),Form("spillnum==%d",spill.at(i)),"lsame");
...
c1->Print(Form("plots/r72_nuabort_run%07d_angle.eps",run2plot));
pfile->Close();
}
