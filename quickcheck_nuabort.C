...
TH2F *hesmx_all = new TH2F("hesmx_all","",23,0,23,100,-5,5);
...
c1->cd();
if (i==0)anabeam->Draw("esmx:Iteration$+1>>hesmx_all","","");
else anabeam->Draw("esmx:Iteration$+1>>hesmx_all","","same");

hesmx_all->GetYaxis()->SetRangeUser(-5,5);
hesmx_all->SetTitle();
hesmx_all->GetXaxis()->SetTitle("ESM monitors");
hesmx_all->GetYaxis()->SetTitle("ESMX");
hesmx->SetLineWidth(2);
//c1->cd();
if (i==0)hesmx_all->Draw("SCAT");
...
else anabeam->Draw("esmx:Iteration$+1>>hesmx",Form("spillnum==%d",spill.at(i)),"lsame");
...
c1->Update();
c1->Print("plots/r71_nuabort_esmx.eps");
pfile->Close();
}
