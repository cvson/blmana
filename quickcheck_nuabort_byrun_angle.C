void quickcheck_nuabort_byrun_angle(Int_t run2plot){
...
//TH2F *haxis = new TH2F("hesmx_all0","",100,-3,3,100,-0.3e-3,0.3e-3);
...
anabeam->Draw(Form("(esmx[0]-esmx[1])/%f:esmx[0]>>haxis(100,-3,3,100,-0.3e-3,0.3e-3)",esm_2_1_pos),"ssemx[18]!=0","colz");
haxis->SetTitle();
haxis->GetXaxis()->SetTitle("ESM 1 X pos.");
haxis->GetYaxis()->SetTitle("Beam angle ");
//hesmx_all0->GetYaxis()->SetRangeUser(-3,3);
//hesmx_all0->SetTitle();
...
anabeam->Draw(Form("(esmx[0]-esmx[1])/%f:esmx[0]>>haxis",esm_2_1_pos),Form("spillnum==%d",spill.at(i)),"p same");
//hesmx->SetLineColor(gStyle->GetColorPalette(theColor));
//hesmx->SetMarkerColor(gStyle->GetColorPalette(theColor));
//hesmx->Draw("p same");
leg0->AddEntry(anabeam,Form("Run %06d Spill%06d ",run.at(i),spill.at(i)),"p");
c1->cd();
c1->Modified();
}
//
...
c1->Modified();
leg0->Draw("same");
c1->Print(Form("plots/r71_nuabort_run%07d_angle.eps",run2plot));
pfile->Close();
}
