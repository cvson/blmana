void t2kpre_quickcheck_nuabort_byrun_ssemx(Int_t run2plot){
...
//TH2F *hssemx = new TH2F("hssemx","",9,0,9,100,-10,10);
//TH2F *hssemx_all = new TH2F("hssemx_all","",9,0,9,100,-10,10);
//TH2F *haxis = new TH2F("hssemx_all0","",9,0,9,100,-10,10);
...
c1->cd();
pchain->Draw("ssemx_bunch[18]:Iteration$+1>>haxis(18,0,9,100,-10,10)","ssemx[18]!=0","colz");
haxis->SetTitle();
haxis->GetXaxis()->SetTitle("SSEM 19 by bunch");
haxis->GetYaxis()->SetTitle("SSEM 19 X pos. [mm]");
...
c1->cd();
anabeam->Draw("ssemx_bunch[18]:Iteration$+1>>hssemx",Form("spillnum==%d",spill.at(i)),"lsame");
leg0->AddEntry(hssemx,Form("Run %06d Spill%06d ",run.at(i),spill.at(i)),"l");
c1->cd();
c1->Modified();
}
//
...
c1->Modified();
leg0->Draw("same");
//c1->Print(Form("plots/r71_nuabort_run%07d_ssemx.eps",run2plot));
c1->Print(Form("plots/r72_nuabort_run%07d_ssemx.eps",run2plot));
pfile->Close();
}
