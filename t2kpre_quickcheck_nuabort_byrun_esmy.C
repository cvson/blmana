void t2kpre_quickcheck_nuabort_byrun_esmy(Int_t run2plot){
...
//TH2F *hesmy_all = new TH2F("hesmy_all","",46,0,23,100,-5,5);
//TH2F *haxis = new TH2F("hesmy_all0","",46,0,23,100,-5,5);
...
c1->cd();
pchain->Draw("esmy:Iteration$+1.0>>haxis(46,0,23,100,-5,5)","ssemx[18]!=0","colz");
haxis->SetTitle();
haxis->GetXaxis()->SetTitle("ESM monitors");
...
c1->cd();
anabeam->Draw("esmy:Iteration$+1>>hesmy",Form("spillnum==%d",spill.at(i)),"lsame");
...
c1->Modified();
leg0->Draw("same");
//c1->Print(Form("plots/r71_nuabort_run%07d_esmy.eps",run2plot));
c1->Print(Form("plots/r72_nuabort_run%07d_esmy.eps",run2plot));
pfile->Close();
}
